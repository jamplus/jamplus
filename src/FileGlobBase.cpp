...
void FileGlobBase::MatchPattern( const char* inPattern )
{
	...
	*destPtr = 0;
	...
	if ( *srcPtr == '@' )
	{
		...
		m_detail->m_ignorePatterns.clear();
		AddIgnorePattern( "./" );
		AddIgnorePattern( "../" );

		m_detail->m_exclusiveFilePatterns.clear();
	}
...
{
	for ( StringList::iterator it = m_detail->m_exclusiveFilePatterns.begin();
			it != m_detail->m_exclusiveFilePatterns.end(); ++it )
	{
#if defined(WIN32)
	...
	m_detail->m_exclusiveFilePatterns.push_back( pattern );
}
...
{
	for ( StringList::iterator it = m_detail->m_ignorePatterns.begin();
			it != m_detail->m_ignorePatterns.end(); ++it )
	{
#if defined(WIN32)
		if ( stricmp( (*it).c_str(), pattern ) == 0 )
#else
	...
	m_detail->m_ignorePatterns.push_back( pattern );
}
	...
	*basePathEndPtr = 0;
	...
	if ( handle != -1 )
	{
		for ( ;; )
		{
			...
			if ( ( finfo.attrib & _A_SUBDIR )  &&  !matchFiles )
			{
				// Do a wildcard match.
				if ( WildMatch( matchPattern, finfo.name, false ) )
				{
					...
					if ( !ignore )
					{
						...
						fileList.push_back( finfo.name );
					}
				}
			}
			else if ( !( finfo.attrib & _A_SUBDIR )  &&  matchFiles )
			{
				// Do a wildcard match.
				if ( WildMatch( matchPattern, finfo.name, false ) )
				{
